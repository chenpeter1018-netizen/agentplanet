...
function uvBinDir() {
  if (isWindows) {
    ...
    if (appdata) return path.join(appdata, 'agent-planet', 'bin')
    return path.join(homedir(), '.agent-planet', 'bin')
  }
  if (isMac) return path.join(homedir(), 'Library', 'Application Support', 'agent-planet', 'bin')
  return path.join(homedir(), '.local', 'share', 'agent-planet', 'bin')
}
...
let DEVICE_KEY_FILE = path.join(OPENCLAW_DIR, 'agent-planet-device-key.json')
...
const CLUSTER_TOKEN = 'agent-planet-cluster-secret-2026'
const PANEL_CONFIG_PATH = path.join(DEFAULT_OPENCLAW_DIR, 'agent-planet.json')
...
const DEFAULT_OPENCLAW_IMAGE = 'ghcr.io/Agent Planet Systems/openclaw'
const PANEL_VERSION = (() => {
  try {
...
function applyOpenclawPathConfig(panelConfig) {
  ...
  DEVICE_KEY_FILE = path.join(OPENCLAW_DIR, 'agent-planet-device-key.json')
...
function detectWindowsShimSource(cliPath) {
  ...
  try {
    ...
    if (lower.includes('@Agent Planet Systems') || lower.includes('openclaw-zh')) return 'npm-zh'
...
function classifyCliSource(cliPath) {
  ...
  if (lower.includes('openclaw-zh') || lower.includes('@Agent Planet Systems')) return 'npm-zh'
  if (isWindows) {
...
function readVersionFromInstallation(cliPath) {
  ...
  const pkgNames = (cliSource === 'standalone' || cliSource === 'npm-zh')
    ? [path.join('@Agent Planet Systems', 'openclaw-zh'), 'openclaw']
    : ['openclaw', path.join('@Agent Planet Systems', 'openclaw-zh')]
...
async function _tryR2Install(version, source, logs) {
  ...
  if (useTarball) {
    ...
    const qcDir = path.join(modulesDir, '@Agent Planet Systems')
    ...
    // 归档内目录可能是 Agent Planet Systems/（Windows tar 不支持 @ 前缀），需要重命名
    const noAtDir = path.join(modulesDir, 'Agent Planet Systems')
    if (fs.existsSync(noAtDir) && !fs.existsSync(qcDir)) {
      fs.renameSync(noAtDir, qcDir)
      logs.push('目录已修正: Agent Planet Systems → @Agent Planet Systems')
    }
    ...
    const openclawJs = path.join(modulesDir, '@Agent Planet Systems', 'openclaw-zh', 'bin', 'openclaw.js')
...
function npmPackageName(source = 'chinese') {
  return source === 'official' ? 'openclaw' : '@Agent Planet Systems/openclaw-zh'
}
...
function collectLocalSkillRoots(agentSkillsDir) {
  ...
  if (isWindows) {
    const prefix = readWindowsNpmGlobalPrefix() || path.join(process.env.APPDATA || '', 'npm')
    for (const pkg of ['openclaw', path.join('@Agent Planet Systems', 'openclaw-zh')]) {
...
function detectInstalledSource() {
  ...
  if (isMac) {
    ...
    if (bin) {
      ...
      if (lower.includes('openclaw-zh') || lower.includes('@Agent Planet Systems') || lower.includes('/openclaw-bin/') || lower.includes('/opt/openclaw/')) return 'chinese'
      return 'official'
    }
    return 'official'
  }
  if (isWindows) {
    try {
      ...
      if (npmPrefix) {
        ...
        const zhDir = path.join(npmPrefix, 'node_modules', '@Agent Planet Systems', 'openclaw-zh')
  ...
  try {
    const npmBin = isWindows ? 'npm.cmd' : 'npm'
    const out = execSync(`${npmBin} list -g @Agent Planet Systems/openclaw-zh --depth=0 2>&1`, { timeout: 10000, windowsHide: true }).toString()
...
function getLocalOpenclawVersion() {
  ...
  if (isMac) {
    ...
    if (!current) {
      try {
        ...
        if (!current) {
          const pkg = path.join(saDir, 'node_modules', '@Agent Planet Systems', 'openclaw-zh', 'package.json')
  ...
  if (!current && isWindows) {
    try {
      const npmPrefix = readWindowsNpmGlobalPrefix()
      if (npmPrefix) {
        for (const pkg of [path.join('@Agent Planet Systems', 'openclaw-zh'), 'openclaw']) {
...
function isAuthenticated(req) {
  ...
  const token = cookies.agent_planet_session
...
function checkPasswordStrength(pw) {
  ...
  const weak = ['123456', '654321', 'password', 'admin', 'qwerty', 'abc123', '111111', '000000', 'letmein', 'welcome', 'agent-planet', 'openclaw']
...
function stripUiFields(config) {
  ...
  // 清理根层级 Agent Planet 内部字段（version info 等），避免污染 openclaw.json
...
function patchGatewayOrigins() {
  ...
  // 合并：保留用户已有的 origins，只追加 Agent Planet 需要的
...
function winStartGateway() {
  ...
  fs.appendFileSync(logPath, `\n[${timestamp}] [Agent Planet] Starting Gateway on Windows...\n`)
...
function matchesCurrentGatewayOwnerSignature(owner) {
  if (!owner || owner.startedBy !== 'agent-planet') return false
...
function linuxStartGateway() {
  ...
  const timestamp = new Date().toISOString()
  fs.appendFileSync(logPath, `\n[${timestamp}] [Agent Planet] Starting Gateway on Linux...\n`)
...
// 查找 agent-planet-agent.cjs 脚本并注入到容器（.cjs 避免容器内 ESM 冲突）
function findAgentScript() {
  const candidates = [
    path.resolve(__dev_dirname, '../openclaw-docker/full/agent-planet-agent.cjs'),
    path.resolve(__dev_dirname, '../openclaw-docker/full/agent-planet-agent.js'),
    path.resolve(__dev_dirname, '../../openclaw-docker/full/agent-planet-agent.cjs'),
    path.resolve(__dev_dirname, '../../openclaw-docker/full/agent-planet-agent.js'),
    path.resolve(__dev_dirname, '../agent-planet-agent.cjs'),
    path.resolve(__dev_dirname, '../agent-planet-agent.js'),
    path.resolve(__dev_dirname, 'agent-planet-agent.cjs'),
    path.resolve(__dev_dirname, 'agent-planet-agent.js'),
  ]
  for (const p of candidates) {
...
async function injectAgentToContainer(containerId, endpoint, cExecFn, agentScript = null) {
  ...
  if (!source) {
    console.warn('[agent] agent-planet-agent.cjs 未找到，跳过注入')
    return false
  }
  const b64 = Buffer.from(source.content, 'utf8').toString('base64')
  await cExecFn(`echo '${b64}' | base64 -d > /app/agent-planet-agent.cjs`)
...
const handlers = {
  ...
  async docker_agent({ nodeId, containerId, cmd } = {}) {
    ...
    const runAgent = async () => {
      const execResult = await dockerExecRun(
        containerId,
        ['node', '/app/agent-planet-agent.cjs', cmdJson],
        node.endpoint,
        timeout,
      )
      return execResult
  ...
  async docker_init_worker({ nodeId, containerId, role = 'general' } = {}) {
    ...
    try {
      ...
      const ROLE_AGENTS = {
        general: '# 操作指令\n\n你是龙虾军团的步兵，接受统帅通过 Agent Planet 下达的任务指令。\n\n## 规则\n- 收到任务后立即执行，完成后简要汇报结果\n- 如果任务不清楚，先确认再行动\n- 保持回复简洁，重点突出\n- 你有独立的记忆空间，会自动记录重要信息',
        coder: '# 操作指令\n\n你是龙虾军团的突击兵，专精编程作战。\n\n## 规则\n- 收到编程任务后，先分析需求再写代码\n- 代码必须可运行，包含必要的注释\n- 主动进行错误处理和边界检查\n- 如果涉及多个文件，说明修改顺序\n- 完成后给出测试建议\n\n## 专长\n- 全栈开发、API 设计、数据库优化\n- Bug 定位与修复、代码重构\n- 性能优化、安全审计',
        translator: '# 操作指令\n\n你是龙虾军团的翻译官，专精多语言互译。\n\n## 规则\n- 翻译要信达雅，保持原文风格\n- 专业术语保留原文标注\n- 长文分段翻译，保持上下文一致\n- 文学作品注重意境传达\n- 技术文档注重准确性\n\n## 专长\n- 中英日韩法德西等主流语言\n- 技术文档、文学作品、商务邮件',
        writer: '# 操作指令\n\n你是龙虾军团的文书官，专精写作任务。\n\n## 规则\n- 根据场景调整语气和风格\n- 注重结构清晰、逻辑连贯\n- 创意写作要有个性和亮点\n- 技术文档要准确严谨\n- 营销文案要抓住痛点\n\n## 专长\n- 博客文章、技术文档、营销文案\n- 故事创作、剧本、诗歌\n- SEO 优化、社交媒体内容',
    ...
    // 4.5 注入 Agent Planet Agent（容器内专属控制代理）
    try {
      await injectAgentToContainer(containerId, node.endpoint, cExec)
      results.files.push('agent-planet-agent.cjs')
    } catch (e) {
  ...
  async diagnose_gateway_connection() {
    ...
    // 2. 设备密钥
    const t2 = Date.now()
    const keyPath = path.join(ocDir, 'agent-planet-device-key.json')
    ...
    return serverCached('status_summary', 60000, () => {
      try {
        ...
        const version = (() => {
          ...
          try {
            for (const pkgName of ['@Agent Planet Systems/openclaw-zh', 'openclaw']) {
  ...
  async upgrade_openclaw({ source = 'chinese', version, method = 'auto' } = {}) {
    ...
    if (tryStandalone) {
      const githubReleaseBase = `https://github.com/Agent Planet Systems/openclaw-standalone/releases/download/v${ver}`
    ...
    if (!version && recommended) {
      logs.push(`Agent Planet ${PANEL_VERSION} 默认绑定 OpenClaw 稳定版: ${recommended}`)
    }
  ...
  uninstall_openclaw({ cleanConfig = false } = {}) {
    ...
    try { execSync(`${npmBin} uninstall -g @Agent Planet Systems/openclaw-zh 2>&1`, { timeout: 60000, windowsHide: true }) } catch {}
  ...
  create_connect_frame({ nonce, gatewayToken }) {
    ...
    return {
      ...
      params: {
        ...
        locale: 'zh-CN', userAgent: 'Agent Planet/1.0.0 (web)',
      },
    }
  },
  // 数据目录 & 图片存储
  assistant_ensure_data_dir() {
    const dataDir = path.join(OPENCLAW_DIR, 'agent-planet')
  ...
  assistant_save_image({ id, data }) {
    const dir = path.join(OPENCLAW_DIR, 'agent-planet', 'images')
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true })
  ...
  async check_panel_update() {
    const sources = [
      { api: 'https://api.github.com/repos/Agent Planet Systems/agent-planet/releases/latest', releases: 'https://github.com/Agent Planet Systems/agent-planet/releases', name: 'github' },
      { api: 'https://gitee.com/api/v5/repos/QtCodeCreators/agent-planet/releases/latest', releases: 'https://gitee.com/QtCodeCreators/agent-planet/releases', name: 'gitee' },
    ]
    let lastErr = ''
    for (const src of sources) {
      try {
        const resp = await globalThis.fetch(src.api, {
          ...
          headers: { 'User-Agent': 'Agent Planet' },
        })
    ...
    return { latest: null, url: 'https://github.com/Agent Planet Systems/agent-planet/releases', error: lastErr }
  },
  ...
  async configure_hermes({ provider, apiKey, model, baseUrl } = {}) {
    ...
    if (fs.existsSync(configPath)) {
      ...
      configContent = `# Hermes Agent configuration (managed by Agent Planet)\nmodel:\n  default: ${modelStr}\n${baseUrlLine}platform_toolsets:\n  api_server:\n    - hermes-api-server\nterminal:\n  backend: local\nplatforms:\n  api_server:\n    enabled: true\n`
    }
    ...
    const newPairs = [[envKey, apiKey], ['GATEWAY_ALLOW_ALL_USERS', 'true'], ['API_SERVER_KEY', 'agent-planet-local']]
  ...
  async hermes_health_check() {
    ...
    const resp = await globalThis.fetch(url, { signal: AbortSignal.timeout(5000), headers: { 'User-Agent': 'Agent Planet-Web' } })
  ...
  async hermes_api_proxy({ method, path: reqPath, body, headers: customHeaders } = {}) {
    ...
    const opts = { method: method || 'GET', headers: { 'User-Agent': 'Agent Planet-Web' } }
  ...
  async hermes_agent_run({ input, sessionId, conversationHistory, instructions } = {}) {
    ...
    const headers = { 'Content-Type': 'application/json', 'User-Agent': 'Agent Planet-Web' }
    if (apiKey) headers['Authorization'] = `Bearer ${apiKey}`
  ...
  hermes_env_set({ key, value } = {}) {
    ...
    const managed = new Set(handlers._hermesManagedEnvKeys())
    if (managed.has(key)) {
      throw new Error(`'${key}' is managed by Agent Planet; please configure it via the provider setup page`)
    }
    const envPath = path.join(hermesHome(), '.env')
  ...
  async hermes_fetch_models({ baseUrl, apiKey, apiType, provider: _provider } = {}) {
    ...
    const headers = { 'User-Agent': 'Agent Planet-Web' }
    let url
  ...
  hermes_logs_download({ name, saveToDisk = false } = {}) {
    ...
    const outDir = path.join(os.homedir(), 'Downloads', 'Agent Planet')
...
async function _apiMiddleware(req, res, next) {
  ...
  if (cmd === 'auth_login') {
    ...
    res.setHeader('Set-Cookie', `agent_planet_session=${token}; Path=/; HttpOnly; SameSite=Lax; Max-Age=${SESSION_TTL / 1000}`)
    res.setHeader('Content-Type', 'application/json')
  ...
  if (cmd === 'auth_logout') {
    const cookies = parseCookies(req)
    if (cookies.agent_planet_session) _sessions.delete(cookies.agent_planet_session)
    res.setHeader('Set-Cookie', 'agent_planet_session=; Path=/; HttpOnly; Max-Age=0')
    res.setHeader('Content-Type', 'application/json')
    res.end(JSON.stringify({ success: true }))
    return
  }
