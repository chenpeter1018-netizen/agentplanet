...
> 目标：Agent Planet Hermes 引擎视觉 + 功能与官方看齐，保留 editorial luxury 主题。
...
3. **架构**：前端 Vanilla JS + CSS scope，后端走 Agent Planet Rust 命令（部分已有，部分需新增）。
4. **分阶段交付**，每阶段独立 PR，可回滚。

---

## 📊 官方 vs Agent Planet 现状对比

| 页面 | 官方功能 | Agent Planet 现状 | Gap |
|---|---|---|---|
...
- Agent Planet 现有：`src/engines/hermes/pages/*.js`、`src-tauri/src/commands/hermes.rs`
- 设计系统：`design-system/agent-planet/MASTER.md`、`src/engines/hermes/style/hermes.css`
