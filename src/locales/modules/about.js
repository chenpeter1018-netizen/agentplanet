...
export default {
  ...
  desc: _('Agent Planet 版本和项目信息', 'Agent Planet version and project info', 'Agent Planet 版本和項目資訊', 'Agent Planet バージョンとプロジェクト情報', 'Agent Planet 버전 및 프로젝트 정보', 'Thông tin phiên bản và dự án Agent Planet', 'Información de versión y proyecto de Agent Planet', 'Informações de versão e projeto do Agent Planet', 'Информация о версии и проекте Agent Planet', 'Informations de version et de projet Agent Planet', 'Agent Planet-Version und Projektinformationen'),
  ...
  licenseTitle: _('软件注册', 'License Activation', '軟體註冊'),
  licenseDesc: _('请输入注册码激活软件。每个注册码最多绑定 3 台电脑，永久有效。首次激活需联网验证，之后可离线使用。', 'Enter your license key. Each key supports up to 3 machines with lifetime validity. First activation requires internet, then works offline.', '請輸入註冊碼激活軟體。每個註冊碼最多綁定 3 台電腦，永久有效。首次激活需聯網驗證，之後可離線使用。'),
  licenseActivated: _('软件已激活，感谢您的支持！', 'Software activated. Thank you for your support!', '軟體已激活，感謝您的支持！'),
  licenseLicensee: _('授权方', 'Licensee', '授權方'),
  licenseMachines: _('已激活设备', 'Activated Devices', '已激活設備'),
  licenseOnline: _('已联网验证', 'Online Verified', '已聯網驗證'),
  licenseOffline: _('离线模式', 'Offline Mode', '離線模式'),
  licenseExpires: _('{days} 天后到期', 'Expires in {days} days', '{days} 天後到期'),
  licenseFingerprint: _('本机指纹', 'Machine Fingerprint', '本機指紋'),
  licenseActivateBtn: _('激活', 'Activate', '激活'),
  licenseVerifying: _('验证中...', 'Verifying...', '驗證中...'),
  licenseInvalidKey: _('请输入有效的注册码', 'Please enter a valid license key', '請輸入有效的註冊碼'),
  licenseSuccess: _('激活成功！', 'Activated successfully!', '激活成功！'),
  licenseFailed: _('激活失败', 'Activation failed', '激活失敗'),
  licenseHint: _('每个注册码最多绑定 3 台电脑 · 首次激活需联网 · 永久有效', 'Each key supports up to 3 machines · First activation requires internet · Lifetime validity', '每個註冊碼最多綁定 3 台電腦 · 首次激活需聯網 · 永久有效'),
  ...
  versionAvailable: _('Agent Planet v{version} 可用', 'Agent Planet v{version} available', 'Agent Planet v{version} 可用', 'Agent Planet v{version} が利用可能です', 'Agent Planet v{version} 사용 가능', 'Agent Planet v{version} đã có sẵn'),
  ...
  updateCommandHint: _('如果 git pull 失败，可先执行 <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:4px">git checkout -- .</code> 丢弃本地修改。<br>路径请替换为实际的 Agent Planet 安装目录。', 'If <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:4px">git pull</code> fails, run <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:4px">git checkout -- .</code> first to discard local changes.<br>Replace the path with your actual Agent Planet install directory.', '如果 git pull 失敗，可先執行 <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:4px">git checkout -- .</code> 捨棄本地修改。<br>路徑請替換為實際的 Agent Planet 安裝目錄。', 'git pull に失敗した場合は、先に <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:4px">git checkout -- .</code> を実行してローカル変更を破棄してください。<br>パスは実際の Agent Planet インストールディレクトリに置き換えてください。', 'git pull 이 실패하면 먼저 <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:4px">git checkout -- .</code> 를 실행해 로컬 변경을 버리세요.<br>경로는 실제 Agent Planet 설치 디렉터리로 바꿔 주세요.', 'Nếu git pull thất bại, hãy chạy <code style="background:var(--bg-tertiary);padding:2px 6px;border-radius:4px">git checkout -- .</code> trước để bỏ các thay đổi cục bộ.<br>Hãy thay đường dẫn bằng thư mục cài đặt Agent Planet thực tế của bạn.'),
  ...
  projectAgentPlanet: _('OpenClaw 可视化管理面板，Tauri v2 桌面应用', 'OpenClaw visual management panel, Tauri v2 desktop app', 'OpenClaw 可视化管理面板，Tauri v2 桌面應用'),
  ...
  domesticMirrorHint: _('国内镜像：<a href="https://gitee.com/QtCodeCreators/agent-planet" target="_blank" rel="noopener" style="color:var(--accent)">Gitee</a>（无法访问 GitHub 时可用）', 'China mirror: <a href="https://gitee.com/QtCodeCreators/agent-planet" target="_blank" rel="noopener" style="color:var(--accent)">Gitee</a> (use when GitHub is inaccessible)', '國內鏡像：<a href="https://gitee.com/QtCodeCreators/agent-planet" target="_blank" rel="noopener" style="color:var(--accent)">Gitee</a>（無法訪問 GitHub 時可用）'),
  ...
  companyDesc: _('我们是 OpenClaw 汉化版（3000+ Star）和 Agent Planet 的作者团队。日常做 AI Agent 相关的产品和开源工具，也接企业私有化部署、定制开发之类的活儿。有事直接群里找我们就行。', 'We are the team behind OpenClaw Chinese Translation (3000+ Stars) and Agent Planet. We build AI Agent products and open source tools, and offer enterprise deployment and custom development services.', '我們是 OpenClaw 漢化版（3000+ Star）和 Agent Planet 的作者團队。日常做 AI Agent 相關的產品和開源工具，也接企業私有化部署、定制開發之類的活儿。有事直接群里找我們就行。'),
