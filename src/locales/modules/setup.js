...
export default {
  ...
  macNodeHint: _('macOS 上从 Finder 启动可能找不到 Node.js。试试关掉 Agent Planet 后从终端启动：', 'On macOS, launching from Finder may not find Node.js. Try closing Agent Planet and launching from terminal:', 'macOS 上從 Finder 啟動可能找不到 Node.js。試試關掉 Agent Planet 后從終端啟動：'),
  ...
  methodHintR2: _('从 CDN 下载独立安装包，自带 Node.js 运行时，无需 npm。国内下载速度最快。', 'Downloads standalone package from CDN with bundled Node.js runtime, no npm needed. Fastest for China.', '從 CDN 下載獨立安裝包，自帶 Node.js 執行時，無需 npm。國內下載速度最快。'),
  ...
  envHintDesc: _('Agent Planet 桌面版只能直接管理<strong>当前 Windows / macOS / Linux 本机</strong>里的 OpenClaw。若你的 OpenClaw 实际装在 WSL、Docker 或远程服务器里，请把下面内容视为<strong>高级替代方案</strong>，而不是普通本机安装的默认路径。', 'Agent Planet desktop can only directly manage OpenClaw installed on the <strong>current local Windows / macOS / Linux machine</strong>. If your OpenClaw actually lives in WSL, Docker or a remote server, treat the options below as <strong>advanced alternatives</strong>, not the default local setup path.', 'Agent Planet 桌面版只能直接管理<strong>目前 Windows / macOS / Linux 本機</strong>裡的 OpenClaw。若你的 OpenClaw 實際裝在 WSL、Docker 或遠端伺服器裡，請把下面內容視為<strong>進階替代方案</strong>，而不是普通本機安裝的預設路徑。'),
  ...
  envHintInstallManage: _('高级替代方案：在对应环境里使用 Agent Planet Web 版', 'Advanced alternative: use Agent Planet Web inside that environment', '進階替代方案：在對應環境裡使用 Agent Planet Web 版'),
  ...
  dockerDefaultImagePlaceholder: _('例如 ghcr.io/Agent Planet Systems/openclaw 或 1186258278/openclaw', 'e.g. ghcr.io/Agent Planet Systems/openclaw or 1186258278/openclaw', '例如 ghcr.io/Agent Planet Systems/openclaw 或 1186258278/openclaw'),
  ...
  wslWebDesc: _('如果你的 OpenClaw 本来就运行在 WSL 里，可打开 WSL 终端部署 Agent Planet Web 版：', 'If your OpenClaw already runs inside WSL, open a WSL terminal and deploy Agent Planet Web there:', '如果你的 OpenClaw 本來就運行在 WSL 裡，可開啟 WSL 終端部署 Agent Planet Web 版：'),
  ...
  dockerDesc: _('如果你的 OpenClaw 本来就运行在容器里，可在容器内安装 OpenClaw + Agent Planet Web 版：', 'If your OpenClaw already runs inside a container, install OpenClaw + Agent Planet Web there:', '如果你的 OpenClaw 本來就運行在容器裡，可在容器內安裝 OpenClaw + Agent Planet Web 版：'),
