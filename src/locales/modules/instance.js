...
export default {
  ...
  remoteHint: _('远程服务器需要运行 Agent Planet (serve.js)。', 'The remote server must be running Agent Planet (serve.js).', '遠程伺服器需要執行 Agent Planet (serve.js)。', 'リモートサーバーで Agent Planet (serve.js) が実行されている必要があります。', '원격 서버에서 Agent Planet (serve.js)이 실행 중이어야 합니다.'),
