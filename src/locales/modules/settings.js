...
export default {
  ...
  autostartToggle: _('系统启动时自动运行 Agent Planet', 'Run Agent Planet automatically on system startup', '系統啟動時自動執行 Agent Planet', 'システム起動時に Agent Planet を自動実行', '시스템 시작 시 Agent Planet 자동 실행'),
  autostartHint: _('开启后，电脑重启时 Agent Planet 会自动启动并检测 Gateway 状态', 'When enabled, Agent Planet will start automatically after reboot and check Gateway status', '開啟後，電腦重啟時 Agent Planet 會自動啟動並檢查 Gateway 狀態', '有効にすると、PC 再起動後に Agent Planet が自動起動し Gateway 状態を確認します', '활성화하면 PC 재시작 후 Agent Planet이 자동으로 시작되어 Gateway 상태를 확인합니다'),
  ...
  dockerDefaultImagePlaceholder: _('例如 ghcr.io/Agent Planet Systems/openclaw 或 1186258278/openclaw', 'e.g. ghcr.io/Agent Planet Systems/openclaw or 1186258278/openclaw', '例如 ghcr.io/Agent Planet Systems/openclaw 或 1186258278/openclaw', '例: ghcr.io/Agent Planet Systems/openclaw または 1186258278/openclaw'),
