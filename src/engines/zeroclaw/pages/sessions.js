...
export function render() {
  ...
  async function loadRows() {
    ...
    try {
      ...
      const settled = await Promise.allSettled(profiles.map(async (profile) => {
        const list = await api.zeroclawSessionsList(null, 80, profile)
  ...
  async function loadDetail(key, redraw = true) {
    ...
    if (redraw) draw()
    try {
      const detail = await api.zeroclawSessionDetail(session.id, session.profile)
  ...
  async function deleteOne(session) {
    ...
    try {
      ...
      await api.zeroclawSessionDelete(session.id, session.profile)
