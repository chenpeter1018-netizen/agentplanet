...
export function render() {
  ...
  async function gw(path, opts = {}) {
    return await api.zeroclawApiProxy((opts.method || 'GET').toUpperCase(), path, opts.body || null)
  }
  ...
  async function loadJobs() {
    try {
      if (gwOnline) {
        ...
        const data = await api.zeroclawCronList()
  ...
  function bindList() {
    el.querySelector('.zc-cron-create')?.addEventListener('click', () => {
      editingJob = { name: '', schedule: '0 9 * * *', prompt: '' }
      draw()
    })
    el.querySelector('.zc-cron-refresh')?.addEventListener('click', async () => {
      loading = true; draw()
      await loadJobs()
      loading = false; draw()
    })
    el.querySelectorAll('.zc-cron-toggle').forEach(btn => {
    ...
    el.querySelectorAll('.zc-cron-run').forEach(btn => {
      btn.addEventListener('click', async () => {
        btn.disabled = true
        try {
    ...
    el.querySelectorAll('.zc-cron-edit').forEach(btn => {
    ...
    el.querySelectorAll('.zc-cron-del').forEach(btn => {
      btn.addEventListener('click', async () => {
        const job = jobs.find(j => (j.id || j.name) === btn.dataset.id)
  ...
  function bindForm(isEdit, id) {
    el.querySelector('.zc-cron-back')?.addEventListener('click', () => { editingJob = null; errorMsg = ''; draw() })
    el.querySelector('.zc-cron-cancel')?.addEventListener('click', () => { editingJob = null; errorMsg = ''; draw() })

    // Cron shortcut pills
    el.querySelectorAll('.zc-cron-shortcut').forEach(btn => {
      btn.addEventListener('click', () => {
        el.querySelectorAll('.zc-cron-shortcut').forEach(b => b.classList.remove('is-active'))
    ...
    schedInput?.addEventListener('input', () => {
      ...
      updatePreview(val)
      el.querySelectorAll('.zc-cron-shortcut').forEach(b => {
        b.classList.toggle('is-active', b.dataset.expr === val)
      })
    })

    // Save
    el.querySelector('.zc-cron-save')?.addEventListener('click', async () => {
