...
const ICONS = {
  ...
  spinner: `<svg class="zeroclaw-spin" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" width="16" height="16"><path d="M12 2a10 10 0 0110 10"/></svg>`,
...
export function render() {
  ...
  function draw() {
    el.innerHTML = `
      <div class="page-header">
        <h1>ZeroClaw</h1>
        <p style="color:var(--text-secondary);margin-top:4px">${t('engine.zeroclawSetupDesc')}</p>
      </div>
      <div style="max-width:720px">
        ...
        <div style="margin-top:16px;text-align:right">
          <a href="https://zeroclaw.nousresearch.com/docs/getting-started/installation/" target="_blank" rel="noopener"
  ...
  function renderPhaseIndicator() {
    ...
    return `<div class="zeroclaw-phases">${phases.map((p, i) => {
      const cls = i < idx ? 'done' : i === idx ? 'active' : ''
      const clickable = i < idx ? `data-goto-phase="${p.id}" style="cursor:pointer" title="${t('engine.zeroclawPhaseClickHint')}"` : ''
      return `<div class="hermes-phase ${cls}" ${clickable}>
        <span class="zeroclaw-phase-dot">${i < idx ? ICONS.check : i + 1}</span>
        <span class="zeroclaw-phase-label">${p.label}</span>
      </div>`
    }).join('<div class="zeroclaw-phase-line"></div>')}</div>`
  }
  ...
  function renderDetect() {
    const rows = []
    if (!hermesInfo) {
      rows.push(`<div class="zeroclaw-detect-row">${ICONS.spinner} <span>${t('engine.detecting')}</span></div>`)
    } else {
      if (hermesInfo.installed) {
        rows.push(`<div class="zeroclaw-detect-row ok">${ICONS.check} <span>${t('engine.zeroclawFound', { version: hermesInfo.version || '' })}</span></div>`)
        if (hermesInfo.running) {
          rows.push(`<div class="zeroclaw-detect-row ok">${ICONS.check} <span>${t('engine.zeroclawReady')}</span></div>`)
        }
      } else {
        rows.push(`<div class="zeroclaw-detect-row">${ICONS.warn} <span>${t('engine.zeroclawNotFound')}</span></div>`)
      }
    }
    return `<div class="card" style="margin-bottom:16px">
      <div class="card-body" style="padding:24px">
        <p style="color:var(--text-secondary);line-height:1.7;margin:0 0 16px">${t('engine.zeroclawSetupIntro')}</p>
        <div class="zeroclaw-detect-list">${rows.join('')}</div>
      </div>
    </div>`
  }
  ...
  function renderInstall() {
    ...
    ` : `
      <div class="hermes-install-info">
        <div class="zeroclaw-detect-row" style="margin-bottom:6px">${ICONS.check} <span>${t('engine.installInfoUv')}</span></div>
        <div class="zeroclaw-detect-row" style="margin-bottom:6px">${ICONS.check} <span>${t('engine.installInfoCore')}</span></div>
        <div class="zeroclaw-detect-row" style="color:var(--text-tertiary)">
  ...
  function renderGateway() {
    const running = hermesInfo?.running
    return `<div class="card" style="margin-bottom:16px">
      <div class="card-body" style="padding:24px">
        ...
        <div class="zeroclaw-detect-row ${running ? 'ok' : ''}">
          ${running ? ICONS.check : ICONS.warn}
          <span>${running ? t('engine.gatewayRunning', { port: hermesInfo?.port || 18790 }) : t('engine.gatewayStopped')}</span>
        </div>
  ...
  function bind() {
    ...
    el.querySelector('.hermes-gw-next')?.addEventListener('click', () => {
      if (hermesInfo?.running) { phase = 'complete'; draw() }
    ...
    el.querySelector('.hermes-go-dashboard')?.addEventListener('click', async () => {
      ...
      window.location.hash = '#/z/dashboard'
    })
    // 自动滚日志到底
  ...
  async function detect() {
    ...
    try {
      ...
      const hm = await api.checkZeroclaw()
      hermesInfo = hm
  ...
  async function doCustomConnect() {
    ...
    try {
      ...
      // 测试连接
      const health = await api.zeroclawHealthCheck()
  ...
  async function doInstall() {
    ...
    // 监听事件（Tauri 模式下有 zeroclaw-install-log/progress 事件）
    try {
      const { listen } = await import('@tauri-apps/api/event')
      const u1 = await listen('zeroclaw-install-log', (e) => {
      ...
      const u2 = await listen('zeroclaw-install-progress', (e) => {
  ...
  async function doSaveConfig() {
    ...
    try {
      const modelId = (model || 'default').replace(/[^a-zA-Z0-9_-]/g, '-')
      const patches = [
        { op: 'add', path: `providers.models.${modelId}.model`, value: model },
        { op: 'add', path: `providers.models.${modelId}.base-url`, value: baseUrl },
        { op: 'add', path: `providers.models.${modelId}.api-key`, value: apiKey },
      ]
  ...
  async function refreshHermes() {
    invalidate('check_zeroclaw')
    try { hermesInfo = await api.checkZeroclaw() } catch (_) {}
    // 已安装且 Gateway 在运行 → 更新引擎状态并跳转仪表盘
    if (hermesInfo?.installed && hermesInfo?.running) {
      phase = 'complete'
      const engine = getActiveEngine()
      if (engine?.detect) await engine.detect()
      window.location.hash = '#/z/dashboard'
