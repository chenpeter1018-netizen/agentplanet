...
function showRenameModal(current) {
  return new Promise((resolve) => {
    ...
    const input = overlay.querySelector('.zc-chat-rename-input')
    input?.focus()
    input?.select()
...
function showContextMenu(x, y, items) {
  const existing = document.querySelector('.zc-chat-ctxmenu')
  ...
  menu.addEventListener('click', (e) => {
    const btn = e.target.closest('.zc-chat-ctxmenu-item')
    if (!btn) return
...
export function render() {
  ...
  el.className = 'zeroclaw-chat-page'
  ...
  api.checkZeroclaw().then(info => {
    gwOnline = !!info?.running
        ...
        <div class="hm-chat-profile-menu" role="menu">
          ...
          ${profiles.map(p => `
            <button class="hm-chat-profile-item ${p.name === active ? 'is-active' : ''}"
              ...
              ${p.running ? `<span class="hm-chat-profile-item-badge">${escHtml(t('engine.chatProfileRunning'))}</span>` : ''}
  ...
  function draw() {
    const scrollTop = el.querySelector('.zc-chat-messages')?.scrollTop
    ...
    // Restore / auto-scroll.
    const msgsEl = el.querySelector('.zc-chat-messages')
    if (msgsEl) {
  ...
  function isMessagesNearBottom(threshold = 120) {
    const m = el.querySelector('.zc-chat-messages')
    if (!m) return true
  ...
  function bind() {
    // --- Sidebar header ---
    el.querySelector('.zc-chat-new-btn')?.addEventListener('click', () => {
      store.newChat()
    })
    ...
    // --- Group collapse ---
    el.querySelectorAll('.zc-chat-group-head[data-group]').forEach(btn => {
    ...
    // --- Session select ---
    el.querySelectorAll('.zc-chat-session-item').forEach(item => {
      item.addEventListener('click', (e) => {
        if (e.target.closest('.zc-chat-session-action')) return
    ...
    el.querySelectorAll('.zc-chat-session-menu').forEach(btn => {
      btn.addEventListener('click', (e) => {
        e.stopPropagation()
    ...
    // --- Session delete ---
    el.querySelectorAll('.zc-chat-session-del').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        e.stopPropagation()
    ...
    el.querySelectorAll('.zc-chat-code-copy').forEach(btn => {
      btn.addEventListener('click', async (e) => {
        e.stopPropagation()
    ...
    el.querySelectorAll('.zc-chat-slash-item').forEach(item => {
  ...
  async function handleSend() {
    ...
    if (text === '/memory' || text === '/skills') {
      store.pushLocalUser(text)
      const target = text === '/memory' ? '/z/memory' : '/z/skills'
      store.pushLocalAssistant(
  ...
  function drawSearchModal() {
    ...
    searchOverlay.innerHTML = `
      <div class="hm-chat-search-panel" data-engine="zeroclaw">
    ...
    searchOverlay.querySelectorAll('.zc-chat-search-item').forEach(btn => {
      btn.addEventListener('click', () => {
      ...
      btn.addEventListener('mouseenter', () => {
        ...
        searchOverlay.querySelectorAll('.zc-chat-search-item').forEach(b =>
  ...
  function onGlobalClick(e) {
    ...
    if (!el.isConnected) return
    const wrap = el.querySelector('.zc-chat-sidebar-profile')
