 ...
 *
 * Agent Planet calls the equivalent Rust/Web-stub commands (`hermes_memory_read_all`
...
export function render() {
  ...
  el.className = 'zeroclaw-memory-page'
  ...
  async function loadAll() {
    ...
    draw()
    try {
      const res = await api.zeroclawMemoryListAll()
  ...
  function cancelEdit() {
    ...
    if (dirty && !confirm(t('engine.memoryUnsaved'))) return
    editing = null
    document.querySelector('.zc-mem-modal-overlay')?.remove()
    draw()
  }
  ...
  async function save() {
    ...
    try {
      await api.zeroclawMemoryWrite(key, buffer)
      data[key] = buffer
  ...
  function bind() {
    el.querySelector('.zc-mem-refresh')?.addEventListener('click', () => loadAll())
    el.querySelectorAll('.zc-mem-edit').forEach(btn => {
