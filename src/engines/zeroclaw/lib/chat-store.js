 ...
 *
 * Responsibilities:
 *   - Load sessions from the backend (via `api.zeroclawSessionsList`) and merge
...
function createStore() {
  ...
  async function loadSessions() {
    ...
    notify()
    try {
      const list = await api.zeroclawSessionsList()
  ...
  async function loadProfiles() {
    ...
    notify()
    try {
      // ZeroClaw: single default profile (no multi-profile support)
      const profiles = [{ name: 'default', active: true }]
      state.profiles = profiles
      const active = state.activeProfile || 'default'
  ...
  async function switchProfile(name) {
    ...
    // ZeroClaw: single profile, just update local state
  ...
  async function refreshActiveMessages() {
    ...
    try {
      const detail = await api.zeroclawSessionDetail(sid)
      if (!detail) return
  ...
  async function deleteSession(sessionId) {
    ...
    if (target && target.source !== '__local__') {
      await api.zeroclawSessionDelete(sessionId)
    }
    ...
    for (const sid of ids) {
      ...
      try {
        if (target.source !== '__local__') {
          await api.zeroclawSessionDelete(sid)
        }
        deleted.push(sid)
      } catch (e) {
  ...
  async function renameSession(sessionId, title) {
    ...
    // ZeroClaw: remote rename not supported, local-only
    if (target.source !== '__local__') {
      // Session title update is local-only in ZeroClaw
    }
  ...
  async function attachStreamListeners(runSessionId) {
    ...
    const u1 = await tauriListen('zeroclaw-chat-chunk', (e) => {
    ...
    const u2 = await tauriListen('zeroclaw-chat-tool', (e) => {
      const evt = e?.payload || {}
      // ZeroClaw WebSocket: { name, args, status: "running" } or { name, content, status: "done" }
      const evtType = evt.status === 'running' ? 'tool.started' : 'tool.done'
      const toolName = evt.name || 'tool'
      const preview = evt.content || evt.preview || evt.detail || ''
      ...
      if (evtType === 'tool.started') {
        const input = extract(evt, ['args', 'input', 'arguments', 'parameters', 'params', 'data'])
      ...
      } else if (evtType === 'tool.done') {
        const t = state.liveTools.find(x => x.name === toolName && x.status === 'running')
        if (t) {
          t.status = 'done'
          t.preview = preview
          t.result = extract(evt, ['content', 'output', 'result', 'data', 'response'])
          if (!t.args) t.args = extract(evt, ['args', 'input', 'arguments', 'parameters', 'params'])
        }
      }
      notify()
    })
    const u3 = await tauriListen('zeroclaw-chat-done', () => {
      const s = runSession()
    ...
    const u4 = await tauriListen('zeroclaw-chat-error', (e) => {
  ...
  async function sendMessage(content, opts = {}) {
    ...
    try {
      ...
      // ZeroClaw WebSocket: session_id + message only (conversation managed server-side)
      await api.zeroclawChatSend(s.id, text)
    } catch (e) {
      s.messages.push({
        id: uid(),
        role: 'system',
