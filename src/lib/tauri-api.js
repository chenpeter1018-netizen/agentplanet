...
async function webInvoke(cmd, args) {
  ...
  if (resp.status === 401) {
    ...
    if (!isTauriRuntime() && window.__agent_planet_show_login) window.__agent_planet_show_login()
...
export const api = {
  ...
  configureHermes: (provider, apiKey, model, baseUrl, opts) => invoke('configure_hermes', {
    ...
    thinkLevel: opts?.think || null, temperature: opts?.temperature ? parseFloat(opts.temperature) : null,
    maxTokens: opts?.maxTokens ? parseInt(opts.maxTokens) : null,
  }),
