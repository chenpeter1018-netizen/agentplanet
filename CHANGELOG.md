...
- **Hermes .env 高级编辑** — 新增非托管环境变量编辑页 `/h/env`，三个新 Tauri 命令（`hermes_env_read_unmanaged` / `hermes_env_set` / `hermes_env_delete`）严格拒绝触碰 Agent Planet 托管的 Provider 密钥，敏感值在 UI 中自动遮罩
...
- **Boot Manifest 启动页改版** — `index.html` 启动页升级为 IDE/Linear/Vercel 风格 manifest 列表，4 步骤错峰淡入动画，支持 zh/en 双语切换并通过 `agent-planet-lang-change` 与应用 i18n 同步
...
- **日志下载反馈** — 桌面端日志下载保存到系统 `Downloads/Agent Planet` 并显示真实路径；Web 端继续浏览器下载并明确提示查看默认下载目录
...
- **Agent Planet 误杀外部 Gateway** — `cleanup_zombie_gateway_processes` 不再杀死外部启动的健康 Gateway 进程，改为采纳其 PID
...
- **Docker 双容器冲突** — 新增 `DISABLE_GATEWAY_SPAWN` 环境变量，禁止 Agent Planet 容器启动本地 Gateway (fixes #159)
...
- **Ollama 配置覆盖** — Agent Planet 不再将用户手动配置的 `api: "ollama"` 覆盖为 `openai-completions` (fixes #140)
...
- **命令执行权限管理** — AI 助手执行终端命令时支持白名单/黑名单规则，四种模式（确认/白名单/黑名单/无限），glob 通配符匹配，存储于 `agent-planet.json`。详见原 `docs/ROADMAP-v0.9.md`
...
- **Gateway 启动失败 Unknown config keys** — stripUiFields 现在清理根层级的 Agent Planet 内部字段（version info），防止污染 openclaw.json
...
- **Gitee 国内镜像** — 部署脚本、项目链接、贡献页面全面接入 Gitee 镜像（gitee.com/QtCodeCreators/agent-planet），国内用户无需翻墙
...
- **Linux Gateway 服务管理不可用 (#7, #10)** — 新增 `linuxCheckGateway()`（ss → lsof → /proc/net/tcp 三级 fallback）、`linuxStartGateway()`（detached 子进程）、`linuxStopGateway()`（SIGTERM），所有 handler 分支加入 Linux 支持；修复 `reload_gateway` / `restart_gateway` 错误执行 `systemctl restart agent-planet`（重启面板而非 Gateway）的问题
...
- **非 root 用户无法部署 Agent Planet (#9)** — `linux-deploy.sh` 支持非 root 安装：普通用户安装到 `$HOME/.local/share/agent-planet`，使用 user-level systemd 服务 + `loginctl enable-linger`；系统包安装通过 `run_pkg_cmd()` 按需 sudo
...
- **Agent 管理/记忆文件页面晦涩错误** — `No such file or directory (os error 2)` 替换为中文提示「OpenClaw CLI 未找到，请确认已安装并重启 Agent Planet」
...
- **Agent Planet Web 版部署文档** — 新增 Linux 一键部署脚本和 Docker 部署指南，官网增加文档中心
...
- **手动指定 Node.js 路径** — 用户可手动输入 Node.js 安装目录，检测通过后自动保存到 `~/.openclaw/agent-planet.json`，后续所有命令自动使用
- **跨平台检测引导** — 安装引导页 Node.js 检测失败时，macOS 提示从终端启动，Windows 提示重启 Agent Planet 或检查 PATH
...
- **Agent Planet 自动更新检测** — 关于页面自动检查 Agent Planet 最新版本，显示更新链接
