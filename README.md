<p align="center">
  <img src="public/images/logo-brand.png" width="360" alt="Agent Planet">
</p>
...
<p align="center">
  <a href="https://github.com/Agent Planet Systems/agent-planet/releases/latest">
    <img src="https://img.shields.io/github/v/release/Agent Planet Systems/agent-planet?style=flat-square&color=6366f1" alt="Release">
  </a>
  <a href="https://github.com/Agent Planet Systems/agent-planet/releases/latest">
    <img src="https://img.shields.io/github/downloads/Agent Planet Systems/agent-planet/total?style=flat-square&color=8b5cf6" alt="Downloads">
  </a>
  <a href="https://github.com/Agent Planet Systems/agent-planet/blob/main/LICENSE">
    <img src="https://img.shields.io/badge/license-AGPL--3.0-blue.svg?style=flat-square" alt="License">
  </a>
  <a href="https://github.com/Agent Planet Systems/agent-planet/actions/workflows/ci.yml">
    <img src="https://img.shields.io/github/actions/workflow/status/Agent Planet Systems/agent-planet/ci.yml?style=flat-square&label=CI" alt="CI">
  </a>
</p>
...
<p align="center">
  <img src="docs/feature-showcase.gif" width="800" alt="Agent Planet 功能全景">
</p>
...
Agent Planet 是支持多 AI Agent 框架的可视化管理面板，目前支持 [OpenClaw](https://github.com/1186258278/OpenClawChineseTranslation) 和 [Hermes Agent](https://github.com/nousresearch/hermes-agent) 双引擎。**内置智能 AI 助手**，帮你一键安装、自动诊断配置、排查问题、修复错误。8 大工具 + 4 种模式 + 交互式问答，从新手到老手都能轻松管理。

> 🌐 **官网**: [claw.qt.cool](https://claw.qt.cool/)  |  📦 **下载**: [GitHub Releases](https://github.com/Agent Planet Systems/agent-planet/releases/latest)
...
Agent Planet 提供**纯 Web 版部署模式**（零 GUI 依赖），天然兼容 ARM64 开发板和嵌入式设备：

- **Orange Pi / 树莓派 / RK3588** 等 ARM64 板子 — `npm run serve` 即可运行
- **Docker ARM64 镜像** — `docker run ghcr.io/Agent Planet Systems/openclaw:latest` 开箱即用
...
<p align="center">
  ...
  &nbsp;·&nbsp;
  <a href="https://github.com/Agent Planet Systems/agent-planet/discussions"><strong>Discussions</strong></a>
  &nbsp;·&nbsp;
  <a href="https://github.com/Agent Planet Systems/agent-planet/issues/new"><strong>反馈 Issue</strong></a>
  &nbsp;·&nbsp;
  <a href="https://qt.cool/c/feishu"><strong>飞书群</strong></a>
</p>
...
前往 [Releases](https://github.com/Agent Planet Systems/agent-planet/releases/latest) 页面下载最新版本，根据你的系统选择对应安装包：

### macOS

| 芯片 | 安装包 | 说明 |
|------|--------|------|
| Apple Silicon (M1/M2/M3/M4) | `Agent Planet_x.x.x_aarch64.dmg` | 2020 年末及之后的 Mac |
| Intel | `Agent Planet_x.x.x_x64.dmg` | 2020 年及之前的 Mac |

> 不确定芯片类型？点击左上角  → 关于本机，查看「芯片」一栏。

安装方式：打开 `.dmg` 文件，**先将 Agent Planet 拖入「应用程序」文件夹**，再双击打开。

> **⚠️ 首次打开提示"已损坏"或"无法验证开发者"？** 由于应用未签名，macOS 会拦截。请在终端执行以下命令解除限制：
>
> ```bash
> sudo xattr -rd com.apple.quarantine /Applications/Agent Planet.app
> ```
>
> 或者前往「系统设置 → 隐私与安全性」，找到 Agent Planet 点击「仍要打开」。
>
> 提示 `No such file`？说明没有拖入应用程序文件夹。请先拖入，或改用：
> ```bash
> sudo xattr -rd com.apple.quarantine ~/Downloads/Agent Planet.app
> ```
...
| 格式 | 安装包 | 说明 |
|------|--------|------|
| EXE 安装器 | `Agent Planet_x.x.x_x64-setup.exe` | 推荐，双击安装 |
| MSI 安装器 | `Agent Planet_x.x.x_x64_en-US.msi` | 企业部署 / 静默安装 |

### Linux

| 格式 | 安装包 | 说明 |
|------|--------|------|
| AppImage | `Agent Planet_x.x.x_amd64.AppImage` | 免安装，`chmod +x` 后直接运行 |
| DEB | `Agent Planet_x.x.x_amd64.deb` | Debian / Ubuntu：`sudo dpkg -i *.deb` |
| RPM | `Agent Planet-x.x.x-1.x86_64.rpm` | Fedora / RHEL：`sudo rpm -i *.rpm` |

### Linux 服务器（Web 版）

没有桌面环境？一键部署 Agent Planet Web 版，通过浏览器远程管理 OpenClaw：

```bash
curl -fsSL https://raw.githubusercontent.com/Agent Planet Systems/agent-planet/main/scripts/linux-deploy.sh | bash
```
...
```bash
docker run -d --name agent-planet --restart unless-stopped \
  -p 1420:1420 -v agent-planet-data:/root/.openclaw \
  node:22-slim \
  sh -c "apt-get update && apt-get install -y git && \
    npm install -g @Agent Planet Systems/openclaw-zh --registry https://registry.npmmirror.com && \
    git clone https://github.com/Agent Planet Systems/agent-planet.git /app && \
...
Agent Planet 提供多种升级方式，根据你的安装方式选择对应方案。

### macOS / Windows 桌面版升级

桌面版内置**自动更新机制**，新版本发布后会自动提示升级：

1. 打开 Agent Planet，如有新版本会弹出升级提示
2. 点击「立即升级」，等待下载完成后自动安装重启
3. 也可前往「关于」页面手动检查更新

> **手动升级**：如果自动更新失败，可前往 [Releases](https://github.com/Agent Planet Systems/agent-planet/releases/latest) 下载最新安装包，覆盖安装即可。数据不会丢失。
...
| DEB | `sudo dpkg -i Agent Planet_最新版本_amd64.deb` |
| RPM | `sudo rpm -U Agent Planet-最新版本-1.x86_64.rpm` |
...
```bash
cd /opt/agent-planet  # 替换为实际安装目录
git pull origin main
npm install
npm run build
sudo systemctl restart agent-planet
```

> **升级 OpenClaw**：面板和 OpenClaw 版本需要匹配。可在「服务管理」页面一键升级，或手动执行：
> ```bash
> sudo npm install -g @Agent Planet Systems/openclaw-zh@latest --registry https://registry.npmmirror.com
> ```
...
```bash
cd /path/to/agent-planet  # 包含 docker-compose.yml 的目录
docker compose build --no-cache agent-planet
docker compose up -d agent-planet
```
...
docker stop agent-planet
docker rm agent-planet
...
```bash
docker exec -it agent-planet npm install -g @Agent Planet Systems/openclaw-zh@latest --registry https://registry.npmmirror.com
```
...
```bash
# 1. 删除应用
sudo rm -rf /Applications/Agent Planet.app

# 2. 清理应用数据（可选，删除后配置将丢失）
rm -rf ~/Library/Application\ Support/com.agent-planet.app
rm -rf ~/Library/Caches/com.agent-planet.app
rm -rf ~/Library/Preferences/com.agent-planet.app.plist
```
...
**方式一：通过系统设置**
1. 打开「设置」→「应用」→「已安装的应用」
2. 搜索 **Agent Planet**，点击「卸载」

**方式二：通过控制面板**
1. 打开「控制面板」→「程序」→「卸载程序」
2. 找到 **Agent Planet**，右键「卸载」

**清理残留数据（可选）**：
```powershell
# 删除应用数据
Remove-Item -Recurse -Force "$env:APPDATA\com.agent-planet.app"
Remove-Item -Recurse -Force "$env:LOCALAPPDATA\com.agent-planet.app"
```
...
| DEB | `sudo dpkg -r agent-planet` |
| RPM | `sudo rpm -e Agent Planet` |

清理应用数据（可选）：
```bash
rm -rf ~/.local/share/com.agent-planet.app
rm -rf ~/.config/com.agent-planet.app
```
...
```bash
# 1. 停止并禁用服务
sudo systemctl stop agent-planet
sudo systemctl disable agent-planet

# 2. 删除 systemd 服务文件
sudo rm -f /etc/systemd/system/agent-planet.service
sudo systemctl daemon-reload

# 3. 删除面板代码
sudo rm -rf /opt/agent-planet

# 4. 卸载 OpenClaw（可选）
sudo npm uninstall -g @Agent Planet Systems/openclaw-zh
...
# 2. 删除镜像（可选）
docker rmi $(docker images --filter "reference=*agent-planet*" -q) 2>/dev/null

# 3. 删除数据卷（可选，删除后所有配置将丢失）
docker volume rm agent-planet-data
```

如果使用 Docker Compose：

```bash
cd /path/to/agent-planet
...
<p align="center">
  <img src="docs/quick-stats.gif" width="800" alt="Agent Planet 数据概览">
</p>
...
Agent Planet 内置的 AI 助手不只是聊天机器人——它能**直接操作你的系统**，帮你诊断、修复、甚至提交 PR。
...
```bash
git clone https://github.com/Agent Planet Systems/agent-planet.git
cd agent-planet
npm install
```
...
首次启动 Agent Planet 会自动进入**初始设置**页面，引导你完成环境检测：
...
- **端口** 1420（Agent Planet）+ 18789（Gateway）
...
Agent Planet 支持将 AI 接入多种即时通讯平台，在「消息渠道」页面统一管理。
...
3. 在 Agent Planet「消息渠道」页面选择飞书，填入凭证
4. 点击「校验」确认连接，然后「保存」
...
从 Finder/Dock 启动 Agent Planet 时，应用的 PATH 环境变量可能不包含 Node.js 安装路径。

**v0.4.1 已修复**：自动补充 `/usr/local/bin`、`/opt/homebrew/bin`、`~/.nvm`、`~/.volta` 等常见路径。

临时解决：从终端启动 Agent Planet：

```bash
open /Applications/Agent Planet.app
```
...
文件被其他进程锁定。先关闭 Agent Planet 和所有 Node.js 进程，以管理员身份打开 PowerShell 重装：
...
安装 Node.js 后需要**重启 Agent Planet**，新的 PATH 环境变量才能生效。
...
2. **检查 Base URL** — 不同服务商 URL 格式不同，注意结尾不要多 `/v1`（Agent Planet 会自动处理）
3. **网络问题** — 国内访问 OpenAI 需要代理；DeepSeek / 阿里通义 / Ollama 国内直连
4. **Ollama 特殊处理** — URL 填 `http://127.0.0.1:11434`（不加 `/v1`，Agent Planet 自动补全）
...
npm install
npm run build
sudo systemctl restart agent-planet  # 或 pm2 restart agent-planet
```
...
Agent Planet 桌面端支持前端热更新，更新文件存储在：

| 系统 | 路径 |
|------|------|
| Windows | `%USERPROFILE%\.openclaw\agent-planet\web-update\` |
| macOS / Linux | `~/.openclaw/agent-planet/web-update/` |

如果热更新后界面显示异常或想回退到安装包自带的版本，删除该目录后重启即可：

```bash
# macOS / Linux
rm -rf ~/.openclaw/agent-planet/web-update

# Windows PowerShell
Remove-Item -Recurse -Force "$env:USERPROFILE\.openclaw\agent-planet\web-update"
```

重启 Agent Planet 后将自动使用内嵌的前端资源。
...
| [ClawApp](https://github.com/Agent Planet Systems/clawapp) | 跨平台移动聊天客户端 |
| [cftunnel](https://github.com/Agent Planet Systems/cftunnel) | Cloudflare Tunnel 内网穿透工具 |

## 赞助商

感谢以下赞助商对 Agent Planet 项目的支持：
...
Agent Planet 的成长离不开每一位贡献者的付出。感谢你们让这个项目变得更好！
...
<table>
  <tr>
    <td align="center"><a href="https://github.com/liucong2013"><img src="https://github.com/liucong2013.png?size=80" width="60" height="60" style="border-radius:50%"><br><sub><b>liucong2013</b></sub></a><br><a href="https://github.com/Agent Planet Systems/agent-planet/pull/88">#88</a></td>
    <td align="center"><a href="https://github.com/axdlee"><img src="https://github.com/axdlee.png?size=80" width="60" height="60" style="border-radius:50%"><br><sub><b>axdlee</b></sub></a><br><a href="https://github.com/Agent Planet Systems/agent-planet/pull/58">#58</a></td>
    <td align="center"><a href="https://github.com/ATGCS"><img src="https://github.com/ATGCS.png?size=80" width="60" height="60" style="border-radius:50%"><br><sub><b>ATGCS</b></sub></a><br><a href="https://github.com/Agent Planet Systems/agent-planet/pull/107">#107</a></td>
    <td align="center"><a href="https://github.com/livisun"><img src="https://github.com/livisun.png?size=80" width="60" height="60" style="border-radius:50%"><br><sub><b>livisun</b></sub></a><br><a href="https://github.com/Agent Planet Systems/agent-planet/pull/106">#106</a></td>
    <td align="center"><a href="https://github.com/kiss-kedaya"><img src="https://github.com/kiss-kedaya.png?size=80" width="60" height="60" style="border-radius:50%"><br><sub><b>kiss-kedaya</b></sub></a><br><a href="https://github.com/Agent Planet Systems/agent-planet/pull/101">#101</a> <a href="https://github.com/Agent Planet Systems/agent-planet/pull/94">#94</a></td>
    <td align="center"><a href="https://github.com/wzh4869"><img src="https://github.com/wzh4869.png?size=80" width="60" height="60" style="border-radius:50%"><br><sub><b>wzh4869</b></sub></a><br><a href="https://github.com/Agent Planet Systems/agent-planet/pull/82">#82</a></td>
    <td align="center"><a href="https://github.com/0xsline"><img src="https://github.com/0xsline.png?size=80" width="60" height="60" style="border-radius:50%"><br><sub><b>0xsline</b></sub></a><br><a href="https://github.com/Agent Planet Systems/agent-planet/pull/15">#15</a></td>
    <td align="center"><a href="https://github.com/jonntd"><img src="https://github.com/jonntd.png?size=80" width="60" height="60" style="border-radius:50%"><br><sub><b>jonntd</b></sub></a><br><a href="https://github.com/Agent Planet Systems/agent-planet/pull/18">#18</a></td>
  </tr>
  <tr>
    <td align="center"><a href="https://github.com/octo-patch"><img src="https://github.com/octo-patch.png?size=80" width="60" height="60" style="border-radius:50%"><br><sub><b>octo-patch</b></sub></a><br><a href="https://github.com/Agent Planet Systems/agent-planet/pull/163">#163</a></td>
    <td align="center"><a href="https://github.com/SEVENTEEN-TAN"><img src="https://github.com/SEVENTEEN-TAN.png?size=80" width="60" height="60" style="border-radius:50%"><br><sub><b>SEVENTEEN-TAN</b></sub></a><br><a href="https://github.com/Agent Planet Systems/agent-planet/pull/161">#161</a></td>
    <td align="center"><a href="https://github.com/roc-xie"><img src="https://github.com/roc-xie.png?size=80" width="60" height="60" style="border-radius:50%"><br><sub><b>roc-xie</b></sub></a><br><a href="https://github.com/Agent Planet Systems/agent-planet/pull/162">#162</a></td>
  </tr>
</table>

### 🐛 社区反馈者

感谢以下用户提交 Issue 报告 Bug 或建议功能，帮助 Agent Planet 持续改进：
...
> 如果遗漏了你的贡献，请 [提交 Issue](https://github.com/Agent Planet Systems/agent-planet/issues/new) 告知我们，我们会第一时间补充！
