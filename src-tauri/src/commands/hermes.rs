...
/// Agent Planet 管理的 uv 二进制存放路径
fn uv_bin_dir() -> PathBuf {
    #[cfg(target_os = "windows")]
    {
        let appdata = std::env::var("APPDATA").unwrap_or_default();
        if !appdata.is_empty() {
            return PathBuf::from(appdata).join("agent-planet").join("bin");
        }
        dirs::home_dir()
            .unwrap_or_default()
            .join(".agent-planet")
...
pub async fn configure_hermes(
    ...
    // Agent Planet 使用 HERMES_PROVIDER_REGISTRY (22 providers) 决定 .env key 名和
...
"#
    ...
    // Agent Planet 管理的 key 列表：包含所有 provider 的 api_key_env_vars + base_url_env_vars
    // + Agent Planet 特定的两个 key。换 provider 时这些会被重写或清除。
    ...
    let mut new_pairs: Vec<(String, String)> = vec![
        ...
        ("API_SERVER_KEY".into(), "agent-planet-local".into()),
    ];
...
pub async fn hermes_health_check() -> Result<Value, String> {
    ...
    let client = super::build_http_client(std::time::Duration::from_secs(5), Some("Agent Planet"))
        .map_err(|e| format!("HTTP 客户端创建失败: {e}"))?;
...
pub async fn hermes_api_proxy(
    ...
    let client = super::build_http_client(timeout, Some("Agent Planet"))
        .map_err(|e| format!("HTTP 客户端创建失败: {e}"))?;
...
pub async fn hermes_agent_run(
    ...
    let client = super::build_http_client(std::time::Duration::from_secs(10), Some("Agent Planet"))
        .map_err(|e| format!("HTTP 客户端创建失败: {e}"))?;
    ...
    let sse_client =
        super::build_http_client(std::time::Duration::from_secs(300), Some("Agent Planet"))
...
/// Read an entire log file and save it to the user's Downloads/Agent Planet
...
pub async fn hermes_logs_download(name: String) -> Result<Value, String> {
    ...
    let out_dir = downloads_dir_fallback().join("Agent Planet");
...
//
// Agent Planet's Hermes integration requires `platforms.api_server.enabled: true`
...
// live in ~/.hermes/.env alongside the Agent Planet-managed provider keys.
//
// The three commands below:
//   * `hermes_env_read_unmanaged` — returns every key in .env that is NOT
//      managed by Agent Planet (i.e. not in `hermes_providers::all_managed_env_keys`)
...
pub fn hermes_env_set(key: String, value: String) -> Result<(), String> {
    ...
    if managed.contains(&key.as_str()) {
        return Err(format!(
            "'{key}' is managed by Agent Planet; please configure it via the provider setup page"
        ));
    }
