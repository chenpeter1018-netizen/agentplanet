...
pub fn validate_openclaw_config() -> Result<Value, String> {
            ...
            if let Some(agents) = obj.get("agents") {
                if let Some(agents_obj) = agents.as_object() {
                    ...
                    if agents_obj.contains_key("profiles") {
                        warnings.push(
                            "发现 agents.profiles 字段，上游 schema 未定义此字段，Agent Planet 会自动清理"
                                .to_string(),
                        );
                    }
                    // 检查 agents.list 中的元素
    ...
    Ok(json!({
        ...
        "suggestions": if !ui_fields_found.is_empty() || !unknown_fields.is_empty() {
            vec![
                "UI 专属字段会被 Agent Planet 自动清理，不影响 OpenClaw 运行".to_string(),
...
/// 清理 Agent Planet 内部字段，避免污染 openclaw.json 导致 Gateway 启动失败
...
fn strip_ui_fields(mut val: Value) -> Value {
    if let Some(obj) = val.as_object_mut() {
        // 清理根层级 Agent Planet 内部字段（version info 等）
        ...
        if let Some(npm_bin) = npm_global_bin_dir() {
            ...
            if !shim_path.exists() {
                ...
                let pkgs: &[&str] = if is_zh {
                    &["@Agent Planet Systems/openclaw-zh", "openclaw"]
                } else {
                    &["openclaw", "@Agent Planet Systems/openclaw-zh"]
                };
                for pkg in pkgs {
...
fn detect_source_from_cmd_shim(cmd_path: &std::path::Path) -> Option<String> {
    ...
    // 汉化版标记：@Agent Planet Systems 或 openclaw-zh
    if lower.contains("openclaw-zh") || lower.contains("@Agent Planet Systems") {
...
fn detect_installed_source() -> String {
    ...
    //   "%dp0%\node_modules\@Agent Planet Systems\openclaw-zh\..."     → 汉化版
    ...
    #[cfg(not(any(target_os = "macos", target_os = "windows")))]
    {
        ...
        if let Ok(o) = npm_command()
            .args(["list", "-g", "@Agent Planet Systems/openclaw-zh", "--depth=0"])
            .output()
        {
...
fn npm_package_name(source: &str) -> &'static str {
    match source {
        "official" => "openclaw",
        _ => "@Agent Planet Systems/openclaw-zh",
    }
}
...
async fn try_r2_install(
    ...
    if use_tarball {
        ...
        let qc_dir = modules_dir.join("@Agent Planet Systems");
        if qc_dir.exists() {
        ...
        let no_at_dir = modules_dir.join("Agent Planet Systems");
        if no_at_dir.exists() && !qc_dir.exists() {
            std::fs::rename(&no_at_dir, &qc_dir)
                .map_err(|e| format!("重命名 Agent Planet Systems → @Agent Planet Systems 失败: {e}"))?;
            let _ = app.emit("upgrade-log", "目录已修正: Agent Planet Systems → @Agent Planet Systems");
        }
...
async fn upgrade_openclaw_inner(
    ...
    if try_standalone {
        let github_release_base = format!(
            "https://github.com/Agent Planet Systems/openclaw-standalone/releases/download/v{}",
            ver
        );
    ...
    if requested_version.is_none() {
        if let Some(recommended) = &recommended_version {
            let _ = app.emit(
                "upgrade-log",
                format!(
                    "Agent Planet {} 默认绑定 OpenClaw 稳定版: {}",
                    panel_version(),
                    recommended
                ),
            );
...
async fn uninstall_openclaw_inner(
    ...
    let other_pkg = if source == "official" {
        "@Agent Planet Systems/openclaw-zh"
    } else {
        "openclaw"
    };
...
/// 保存用户自定义的 Node.js 路径到 ~/.openclaw/agent-planet.json
#[tauri::command]
    ...
    match cli_check {
        ...
        _ => {
            return Err("openclaw CLI 未安装。请先执行以下命令安装：\n\n\
                 npm install -g @Agent Planet Systems/openclaw-zh\n\n\
...
/// 检查 Agent Planet 自身是否有新版本（GitHub → Gitee 自动降级）
#[tauri::command]
pub async fn check_panel_update() -> Result<Value, String> {
    let client =
        crate::commands::build_http_client(std::time::Duration::from_secs(8), Some("Agent Planet"))
            .map_err(|e| format!("创建 HTTP 客户端失败: {e}"))?;

    // 先尝试 GitHub，失败后降级 Gitee
    let sources = [
        (
            "https://api.github.com/repos/Agent Planet Systems/agent-planet/releases/latest",
            "https://github.com/Agent Planet Systems/agent-planet/releases",
            "github",
        ),
        (
            "https://gitee.com/api/v5/repos/QtCodeCreators/agent-planet/releases/latest",
            "https://gitee.com/QtCodeCreators/agent-planet/releases",
            "gitee",
        ),
    ];
...
pub async fn test_proxy(url: Option<String>) -> Result<Value, String> {
    ...
    let client =
        crate::commands::build_http_client(std::time::Duration::from_secs(10), Some("Agent Planet"))
