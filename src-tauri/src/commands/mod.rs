...
fn panel_config_candidate_paths() -> Vec<PathBuf> {
    ...
    push_unique_panel_config_path(&mut paths, default_openclaw_dir().join("agent-planet.json"));
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        if let Ok(appdata) = std::env::var("APPDATA") {
            ...
            if let Some(profile_dir) = appdata_path.parent().and_then(|p| p.parent()) {
                push_unique_panel_config_path(
                    &mut paths,
                    profile_dir.join(".openclaw").join("agent-planet.json"),
                );
            }
        }
    }
...
/// 获取 OpenClaw 配置目录
/// 优先使用 agent-planet.json 中的 openclawDir 自定义路径，不存在则回退默认 ~/.openclaw
...
/// 并尊重 `agent-planet.json` 中的 `openclawDir` 自定义配置目录。
...
fn panel_config_path() -> PathBuf {
    ...
    candidates
        .into_iter()
        .next()
        .unwrap_or_else(|| default_openclaw_dir().join("agent-planet.json"))
}
