...
fn legacy_plugin_backup_dir(plugin_id: &str) -> PathBuf {
    super::openclaw_dir()
        .join("extensions")
        .join(format!("{plugin_id}.__agent-planet_backup"))
}
...
fn is_plugin_builtin(plugin_id: &str) -> bool {
    ...
    let npm_dirs: Vec<PathBuf> = {
        ...
        if let Some(appdata) = std::env::var_os("APPDATA") {
            ...
            dirs.push(base.join("@Agent Planet Systems").join("openclaw-zh"));
            dirs.push(base.join("openclaw"));
        }
        #[cfg(target_os = "macos")]
        {
            dirs.push(PathBuf::from(
                "/opt/homebrew/lib/node_modules/@Agent Planet Systems/openclaw-zh",
            ));
            dirs.push(PathBuf::from("/opt/homebrew/lib/node_modules/openclaw"));
            dirs.push(PathBuf::from(
                "/usr/local/lib/node_modules/@Agent Planet Systems/openclaw-zh",
            ));
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            dirs.push(PathBuf::from("/usr/local/lib/node_modules/openclaw"));
            dirs.push(PathBuf::from(
                "/usr/lib/node_modules/@Agent Planet Systems/openclaw-zh",
            ));
...
fn generic_plugin_backup_dir(plugin_id: &str) -> PathBuf {
    plugin_backup_root().join(format!("{plugin_id}.__agent-planet_backup"))
}
...
pub async fn install_channel_plugin(
    ...
    if !status.success() {
        ...
        if is_host_version_issue {
            ...
            let _ = app.emit("plugin-log", "  npm i -g @Agent Planet Systems/openclaw-zh@latest --registry https://registry.npmmirror.com");
        }
        let rollback_err =
            cleanup_failed_plugin_install(plugin_id, had_existing_plugin, had_existing_config)
                .err()
