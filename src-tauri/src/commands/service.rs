...
fn matches_current_gateway_owner_signature(owner: &GatewayOwnerRecord) -> bool {
    if owner.started_by != "agent-planet" {
        return false;
    }
    ...
    fn start_gateway_direct() -> Result<(), String> {
        ...
        cmd.spawn().map_err(|e| {
            if e.kind() == std::io::ErrorKind::NotFound {
                "OpenClaw CLI 未找到，请确认已安装并重启 Agent Planet。".to_string()
            } else {
...
mod platform {
    ...
    fn create_gateway_log_files() -> Result<(std::fs::File, std::fs::File), String> {
        ...
        let _ = writeln!(
            stdout_log,
            "\n[{}] [Agent Planet] Hidden-start Gateway on Windows",
    ...
    pub async fn start_service_impl(_label: &str) -> Result<(), String> {
        if !is_cli_installed() {
            return Err(
                "openclaw CLI 未安装，请先通过 npm install -g @Agent Planet Systems/openclaw-zh 安装"
                    .into(),
            );
        }
