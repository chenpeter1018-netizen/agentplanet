...
fn collect_env() -> DiagnoseEnv {
    ...
    // 设备密钥
    let device_key_path = openclaw_dir.join("agent-planet-device-key.json");
...
fn check_device_key() -> DiagnoseStep {
    let t = step_timer();
    let key_path = crate::commands::openclaw_dir().join("agent-planet-device-key.json");
    if key_path.exists() {
...
//
// This is NOT a Agent Planet bug — we only expose a detection command so the
