...
/// 前端热更新目录 (~/.openclaw/agent-planet/web-update/)
pub fn update_dir() -> PathBuf {
    super::openclaw_dir().join("agent-planet").join("web-update")
}
...
pub async fn download_frontend_update(
    ...
    let client = super::build_http_client(std::time::Duration::from_secs(120), Some("Agent Planet"))
        .map_err(|e| format!("HTTP 客户端错误: {e}"))?;
