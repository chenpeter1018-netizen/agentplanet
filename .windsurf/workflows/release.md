...
- `docs/index.html` — JSON-LD `softwareVersion`、下载链接文件名（`Agent Planet_x.y.z_xxx`）、版本徽标（`vx.y.z 最新版`）
...
Mac 堡垒机上的 Agent Planet 项目需要手动同步（如有需要）：
```bash
ssh mac "cd /Users/admin/Desktop/agent-planet && export https_proxy=http://127.0.0.1:7897 && git pull"
```
