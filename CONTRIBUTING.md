...
感谢你对 Agent Planet 项目的关注！本文档同时作为**贡献指南**和**项目维护手册**，涵盖开发、构建、发版、部署的完整工作流。

> 🌐 **官网**: [claw.qt.cool](https://claw.qt.cool/)  |  📦 **仓库**: [github.com/Agent Planet Systems/agent-planet](https://github.com/Agent Planet Systems/agent-planet)
...
Agent Planet 有两种运行模式，前端代码通过 `isTauri` 标志自动适配：
...
OpenClaw 主配置文件，包含模型配置、网关配置等。由 Agent Planet 的"模型配置"和"网关配置"页面读写。

### `~/.openclaw/agent-planet.json`

Agent Planet 面板自身的配置文件，独立于 OpenClaw：
...
Agent Planet 支持访问密码保护，**Web 模式和 Tauri 桌面端均可启用**：

| 模式 | 密码存储 | 验证方式 | 会话管理 |
|------|----------|----------|----------|
| Web | `agent-planet.json` | 后端比对 + HTTP-only Cookie | 服务端 session（24h TTL） |
| Tauri 桌面 | `agent-planet.json` | 前端本地比对 | `sessionStorage` |

### 密码保护流程

```
启动 → 读 agent-planet.json
  ├─ 无密码 + ignoreRisk → 放行
  ├─ 有密码 + 未认证 → 弹出登录覆盖层
...
面向 macOS / Windows / Linux 桌面用户，从 [GitHub Releases](https://github.com/Agent Planet Systems/agent-planet/releases) 下载安装包。
...
```bash
gh pr list --repo Agent Planet Systems/agent-planet --state all --limit 200 --json author,number,title,state
gh issue list --repo Agent Planet Systems/agent-planet --state all --limit 200 --json author,number,title
```
...
如果发现 Bug 或有功能建议，欢迎通过 [GitHub Issues](https://github.com/Agent Planet Systems/agent-planet/issues) 提交。
